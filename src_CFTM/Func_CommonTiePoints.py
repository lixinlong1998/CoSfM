...
def getEpoch_byFOLDER(cameraPaths):
    '''
    This function will divide them based on the folders where the photos are stored.
    it returns:
        epochs = ['epoch1_20201014', 'epoch2_20210703']
    '''
    datelist = []
    for camera_path, identifier in cameraPaths.items():
        image_folder = os.path.basename(os.path.dirname(camera_path))
        datelist.append(image_folder)
    epochlist = list(set(datelist))
    epochlist.sort()
    epochs = [epoch for epoch in epochlist]
    return epochs
...
def analyseCameras_Epoch(camera_ids, cameraPaths, epochs, epoch_mode):
    '''
    input:
        camera_ids = {identifier:camera_id}
        cameraPaths = {camera_path:identifier}
        epochs = [['2020:10:14'], ['2021:07:03'], ['2021:12:03'], ['2021:12:06']]
