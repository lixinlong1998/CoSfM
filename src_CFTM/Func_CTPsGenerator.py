...
def GenerateCTPs(grid_id, corners, Camera_IdList, Cameras, camera_ids, CamerasEpoch, Sensors,
                 CoordinateTransform, CoordinateAttribute, chunkKeypoints, chunkDescriptors,
                 path_folder, state_CFTM_path, arguments):
    # get arguments
    ...
    for camera_id in Camera_IdList:
        ...
        CamerasMask_cube[identifier] = polygon
    ...
    if exportReport:
        np.save(path_CamerasMask, CamerasMask_cube)
    ...
    if len(features_cube_e1) < 2 or len(features_cube_e2) < 2:
        ...
        with open(os.path.join(state_CFTM_path, f'state_{grid_id}.txt'), 'w') as file:
            file.write(f"Grid_id:{grid_id}")
            file.write(f"State:SKIP")
            file.write(f"Reason:Too less cameras could observe this cube!")
            file.write(f"Cameras number in e1::{len(features_cube_e1)}")
            file.write(f"Cameras number in e2:{len(features_cube_e2)}")
            file.write(f"Number of camera pairs in e1:{0}")
            file.write(f"Number of camera pairs in e2:{0}")
            file.write(f"Length of tracks in e1:{0},{0},{0}")
            file.write(f"Length of tracks in e2:{0},{0},{0}")
            file.write(f"Number of generated tracks in e1:{0}")
            file.write(f"Number of generated tracks in e2:{0}")
            file.write(f"Remaining number of tracks in e1:{0}")
            file.write(f"Remaining number of tracks in e2:{0}")
            file.write(f"Number of Common tracks:{0}")
            file.write(f"Time cost for this cube:{time.perf_counter() - starttime0}")
        return None
    ...
    matches_cube_e1, MatchesReport_cube_e1 = Matc.innerMatch(features_cube_e1, chunkKeypoints, chunkDescriptors,
                                                             Cameras, Sensors, camera_ids, criterias1, exportReport)
    matches_cube_e2, MatchesReport_cube_e2 = Matc.innerMatch(features_cube_e2, chunkKeypoints, chunkDescriptors,
                                                             Cameras, Sensors, camera_ids, criterias1, exportReport)
    ...
    if len(matches_cube_e1) == 0 or len(matches_cube_e2) == 0:
        ...
        with open(os.path.join(state_CFTM_path, f'state_{grid_id}.txt'), 'w') as file:
            file.write(f"Grid_id:{grid_id}")
            file.write(f"State:SKIP")
            file.write(f"Reason:At least one epoch have not valid match!")
            file.write(f"Cameras number in e1::{len(features_cube_e1)}")
            file.write(f"Cameras number in e2:{len(features_cube_e2)}")
            file.write(f"Number of camera pairs in e1:{len(matches_cube_e1)}")
            file.write(f"Number of camera pairs in e2:{len(matches_cube_e2)}")
            file.write(f"Length of tracks in e1:{0},{0},{0}")
            file.write(f"Length of tracks in e2:{0},{0},{0}")
            file.write(f"Number of generated tracks in e1:{0}")
            file.write(f"Number of generated tracks in e2:{0}")
    ...
    # [export]
    if exportReport:
        np.save(path_Matches, [MatchesReport_cube_e1, MatchesReport_cube_e2])
    # print('[Script]    [{}]    [Script][TimeCost]    :'.format(grid_id), time.perf_counter() - starttime1)
    ...
    if len(Tracks_Generated_e1) == 0 or len(Tracks_Generated_e2) == 0:
        print('[Script]    [{}]This cube has no track generated!'.format(grid_id))
        print('[Script]    [{}]Skip grid!'.format(grid_id))
        with open(os.path.join(state_CFTM_path, f'state_{grid_id}.txt'), 'w') as file:
            file.write(f"Grid_id:{grid_id}")
            file.write(f"State:SKIP")
            file.write(f"Reason:At least one epoch have not track!")
            file.write(f"Cameras number in e1::{len(features_cube_e1)}")
            file.write(f"Cameras number in e2:{len(features_cube_e2)}")
            file.write(f"Number of camera pairs in e1:{len(matches_cube_e1)}")
            file.write(f"Number of camera pairs in e2:{len(matches_cube_e2)}")
            file.write(f"Length of tracks in e1:{max(Tracks_length1)},{np.mean(Tracks_length1)},{min(Tracks_length1)}")
            file.write(f"Length of tracks in e2:{max(Tracks_length2)},{np.mean(Tracks_length2)},{min(Tracks_length2)}")
            file.write(f"Number of generated tracks in e1:{len(Tracks_Generated_e1)}")
            file.write(f"Number of generated tracks in e2:{len(Tracks_Generated_e2)}")
            file.write(f"Remaining number of tracks in e1:{0}")
            file.write(f"Remaining number of tracks in e2:{0}")
            file.write(f"Number of Common tracks:{0}")
            file.write(f"Time cost for this cube:{time.perf_counter() - starttime0}")
    ...
    # [export]
    if exportReport:
        TracksReport_cube_e1 = Trac.getOriginTracksReport(Tracks_Generated_e1, Tracks_Matches_e1)
        TracksReport_cube_e2 = Trac.getOriginTracksReport(Tracks_Generated_e2, Tracks_Matches_e2)
        np.save(path_TracksReport, [TracksReport_cube_e1, TracksReport_cube_e2])
    # print('[Script]    [{}]    [Script][TimeCost]    :'.format(grid_id), time.perf_counter() - starttime1)
    ...
    Tracks_Filtered_e1, Tracks_Quality_e1 = Trac.filterTracks(grid_id, Tracks_Generated_e1, Cameras, Sensors,
                                                              ...
                                                              criterias2, threshold_RepError2, show=showScreen)
    ...
    if len(Tracks_Filtered_e1) == 0 or len(Tracks_Filtered_e2) == 0:
        ...
        with open(os.path.join(state_CFTM_path, f'state_{grid_id}.txt'), 'w') as file:
            file.write(f"Grid_id:{grid_id}")
            file.write(f"State:SKIP")
            file.write(f"Reason:None of epoch Track left after filter!")
            file.write(f"Cameras number in e1::{len(features_cube_e1)}")
            file.write(f"Cameras number in e2:{len(features_cube_e2)}")
            file.write(f"Number of camera pairs in e1:{len(matches_cube_e1)}")
            file.write(f"Number of camera pairs in e2:{len(matches_cube_e2)}")
            ...
            file.write(f"Number of generated tracks in e1:{len(Tracks_Generated_e1)}")
            file.write(f"Number of generated tracks in e2:{len(Tracks_Generated_e2)}")
            file.write(f"Remaining number of tracks in e1:{len(Tracks_Filtered_e1)}")
            file.write(f"Remaining number of tracks in e2:{len(Tracks_Filtered_e2)}")
            file.write(f"Number of Common tracks:{0}")
            file.write(f"Time cost for this cube:{time.perf_counter() - starttime0}")
        return None
    # [export]
    if exportReport:
        FilterTracksReport_cube_e1 = Trac.getFilterTracksReport(Tracks_Filtered_e1, Tracks_Matches_e1)
        FilterTracksReport_cube_e2 = Trac.getFilterTracksReport(Tracks_Filtered_e2, Tracks_Matches_e2)
        np.save(path_FilterTracksReport, [FilterTracksReport_cube_e1, FilterTracksReport_cube_e2])
    # print('[Script]    [{}]    [Script][TimeCost]    :'.format(grid_id), time.perf_counter() - starttime1)

    # Step5: Match tracks from different epochs with robust filters
    starttime1 = time.perf_counter()
    # print('[Script]    [{}]    Matching tracks...'.format(grid_id))
    CommonTracks, CommonTracksMatches = Trac.matchTracks(Tracks_Filtered_e1, Tracks_Filtered_e2,
                                                         Tracks_Quality_e1, Tracks_Quality_e2,
                                                         Cameras, Sensors, chunkDescriptors, threshold_Distance,
                                                         crossCheck=True)
    # print('[Script]    [{}]    Number of Common tracks:'.format(grid_id), len(CommonTracks))
    # [Exception Handling]: In case there is no epoch Track left
    if len(CommonTracks) == 0:
        ...
        with open(os.path.join(state_CFTM_path, f'state_{grid_id}.txt'), 'w') as file:
            file.write(f"Grid_id:{grid_id}")
            file.write(f"State:SKIP")
            file.write(f"Reason:There is no common track!")
            file.write(f"Cameras number in e1::{len(features_cube_e1)}")
            file.write(f"Cameras number in e2:{len(features_cube_e2)}")
            file.write(f"Number of camera pairs in e1:{len(matches_cube_e1)}")
            file.write(f"Number of camera pairs in e2:{len(matches_cube_e2)}")
            ...
            file.write(f"Remaining number of tracks in e1:{len(Tracks_Filtered_e1)}")
            file.write(f"Remaining number of tracks in e2:{len(Tracks_Filtered_e2)}")
            file.write(f"Number of Common tracks:{len(CommonTracks)}")
            file.write(f"Time cost for this cube:{time.perf_counter() - starttime0}")
        return None
    # [export]
    if exportReport:
        np.save(path_CommonTracksReport, CommonTracksMatches)
    # print('[Script]    [{}]    [Script][TimeCost]    :'.format(grid_id), time.perf_counter() - starttime1)
    ...
    print('[Script]    [{}]    Number of Marker Points of this cube:'.format(grid_id), len(CommonTracks))
    # print('[Script]    [{}]    [Script][TimeCost]    :'.format(grid_id), time.perf_counter() - starttime1)
    ...
    with open(os.path.join(state_CFTM_path, f'state_{grid_id}.txt'), 'w') as file:
        file.write(f"Grid_id:{grid_id}")
        file.write(f"State:DONE")
        file.write(f"Reason:Successfully find common tracks!")
        file.write(f"Cameras number in e1::{len(features_cube_e1)}")
        file.write(f"Cameras number in e2:{len(features_cube_e2)}")
        file.write(f"Number of camera pairs in e1:{len(matches_cube_e1)}")
        file.write(f"Number of camera pairs in e2:{len(matches_cube_e2)}")
...
def buildMarkersGrid(Boundary_PJCS, girdsize, MarkerPoints, MarkerTracks, MarkerInforms):
    ...
    input:
        ...
        MarkerInforms = [Information,Information,...,Information]
            ,where Information=[[point_id,Track_id],] or [[CrossMatch,...],]
    output:
    ...
    for i, MarkerPoint in enumerate(MarkerPoints):
        ...
        MarkerKey = i * 1e+12 + r * 1e+6 + c
        MarkerInforms[i].append(MarkerKey)
